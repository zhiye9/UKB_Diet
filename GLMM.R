...
like_high = aggregate(item_high[1], list(item_high$subjects), mean, na.rm=TRUE)
like_low = aggregate(item_low[1], list(item_low$subjects), mean, na.rm=TRUE)
colnames(like_high)[1] <- "eid"
colnames(like_low)[1] <- "eid"
like_high$liking = scale(like_high$liking)
like_low$liking = scale(like_low$liking)
write.csv(like_high, 'like_high.csv', row.names = FALSE)
write.csv(like_low, 'like_low.csv', row.names = FALSE)
