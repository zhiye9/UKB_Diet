...
#X_WHR1 = np.concatenate((np.array(df_SCE_gmv_2000[gmv]), np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
#X_WHR2 = np.concatenate((stats.zscore(np.array(IC55)), np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
#X_WHR3 = np.concatenate((np.array(df_SCE_gmv_2000[gmv]), stats.zscore(np.array(IC55)), np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
#X_WHR_GMV_GT = np.concatenate((np.array(df_SCE_gmv_2000[gmv]), stats.zscore(np.array(IC_Graph)), np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
#X_WHR_GT = np.concatenate((stats.zscore(np.array(IC_Graph)), np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
X_WHR_BMI = np.array(df_SCE_gmv_2000[['sex', 'age', 'slope', 'height', 'hand', 'sexage', 'edu', 'IQ', 'income', 'household', 'liking_low', 'liking_high', 'energy'] + list(df_genetic_noNAN.columns[1:])])
...
#Drop features with beta = 0 in any round
...
t1 = [extract_IC_color(ica100_template, i, 1.1) for i in GT_id_pos_corr_pos_neg_corr_pos]
t2 = [extract_IC_color(ica100_template, i, 1.2) for i in GT_id_pos_corr_pos_neg_corr_neg]
t3 = [extract_IC_color(ica100_template, i, 1.3) for i in GT_id_pos_corr_neg_neg_corr_pos]
t4 = [extract_IC_color(ica100_template, i, 1.4) for i in GT_id_pos_corr_neg_neg_corr_neg]
t5 = [extract_IC_color(ica100_template, i, 1.5) for i in GT_id_pos_corr_pos_only]
t6 = [extract_IC_color(ica100_template, i, 1.6) for i in GT_id_pos_corr_neg_only]
t7 = [extract_IC_color(ica100_template, i, 1.7) for i in GT_id_neg_corr_pos_only]
t8 = [extract_IC_color(ica100_template, i, 1.8) for i in GT_id_neg_corr_neg_only]
IC_img = t1 + t2 + t3 + t4 + t5 + t6 + t7 + t8
IC_img = [t1[0], t2[0]]
atlas = image.math_img('np.sum(img, axis = -1)', img = IC_img)
plotting.plot_roi(atlas, cut_coords = [1, -71, 13], cmap = "Spectral", colorbar = True)
oo = np.unique(atlas.get_data(), return_counts = True)

IC1 = t1[0]
IC2 = t2[0]

def avoid_IC_overlap(IC1, IC2):
    data = np.sum([IC1.get_data(), IC2.get_data()], axis = 0)
    data[data > 2] = np.unique(IC1.get_data())[-1]
    return new_img_like(IC1, data)

last_IC = IC_img[0]
for i in range(len(IC_img) -1):
    new_IC_overlap = avoid_IC_overlap(last_IC, IC_img[i + 1])
...
ooo = avoid_IC_overlap(t1[0], t2[0])
plotting.plot_roi(ooo, cut_coords = [1, -71, 13], cmap = "Spectral", colorbar = True)
np.unique(ooo.get_data(), return_counts = True)
