...
IC_img_value = [IC_img[i].get_data() for i in range(len(IC_img))]
...
ica100_template = nib.load('IC_55_atlas_withoutprob.nii.gz')
GM_IC_template_mask_sum = np.multiply(GM_template_mask.get_data(), new_IC_template_mask.get_data())
...
IC_img_value_p = [IC_img_p[i].get_data() for i in range(len(IC_img_p))]
...
IC_img_value_n = [IC_img_n[i].get_data() for i in range(len(IC_img_n))]
...
IC_value_p_p = [t_p_p[i].get_data() for i in range(len(t_p_p))]
IC_p_p_data = np.sum(IC_value_p_p, axis = 0)

IC_value_p_n = [t_p_n[i].get_data() for i in range(len(t_p_n))]
...
GM_IC_pos_net_data = np.sum([GM_intersec_pos_neg.get_data(), last_IC_10_p.get_data()], axis = 0)
...
IC_value_n_p = [t_n_p[i].get_data() for i in range(len(t_n_p))]
IC_n_p_data = np.sum(IC_value_n_p, axis = 0)

IC_value_n_n = [t_n_n[i].get_data() for i in range(len(t_n_n))]
...
GM_IC_pos_net_data = np.sum([GM_intersec_pos_neg.get_data(), last_IC_10_n.get_data()], axis = 0)
GM_IC_pos_net_data_sub = np.subtract(np.multiply(GM_IC_pos_net_data, np.full(GM_IC_pos_net_data.shape, 10)), np.full(GM_IC_pos_net_data.shape, 23))
GM_IC_pos_net_data_sub[GM_IC_pos_net_data_sub < 0] = 0
GM_IC_pos_net = new_img_like(GM_intersec_pos_neg, np.around(GM_IC_pos_net_data_sub))
plotting.plot_roi(GM_IC_pos_net, cut_coords = [-1, -44, 12], cmap = cmap4, colorbar = True)
...
#WHR ~ GMV + Control
X_WHR1 = np.concatenate((np.array(df_SCE_gmv_2000_female[gmv]), np.array(df_SCE_gmv_2000_female[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
GMV_train_WHR, GMV_test_WHR, beta_GMV, GM_models = CV(p_grid = par_grid, out_fold = 5, in_fold = 5, model = ElasticNet(max_iter = 1000000), X = X_WHR1, y = np.array(stats.zscore(df_SCE_gmv_2000_female['waist_hip_ratio'])), n_beta = 147, rand = 9)

#Energy/WHR ~ ICA_GT + Control
X_WHR_GT = np.concatenate((stats.zscore(np.array([IC_Graph[i] for i in df_SCE_gmv_2000_female.index])), np.array(df_SCE_gmv_2000_female[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
WHR_GT_train_E, WHR_GT_test_E, beta_GT, GT_models = CV(p_grid = par_grid, out_fold = 5, in_fold = 5, model = ElasticNet(max_iter = 1000000), X = X_WHR_GT, y = np.array(stats.zscore(df_SCE_gmv_2000_female['waist_hip_ratio'])), n_beta = 122, rand = 9)
...
#plotting.plot_roi(last_IC_10, cut_coords = [1, -71, 13], cmap = "Set1", colorbar = True)
#plotting.plot_roi(last_IC_10, cut_coords = [-1, -44, 12], cmap = "Set1", colorbar = True)
plotting.plot_roi(last_IC_10, cut_coords = [-1, -44, 12], cmap = colors.ListedColormap(['purple',  'yellow', 'green','brown',  'pink', 'grey']), colorbar = True)
#plotting.plot_roi(last_IC_10, cut_coords = [-1, -44, 12], cmap = "Spectral", colorbar = True)
np.unique(last_IC_10.get_data(), return_counts = True)
...
GM_IC_pos_net_data_sub[GM_IC_pos_net_data_sub < 0] = 0
GM_IC_pos_net = new_img_like(GM_intersec_pos_neg, np.around(GM_IC_pos_net_data_sub))
plotting.plot_roi(GM_IC_pos_net, cmap = cmap4, colorbar = True)
print(np.unique(get_data(GM_IC_pos_net), return_counts = True))
...
GM_IC_pos_net_data_sub[GM_IC_pos_net_data_sub < 0] = 0
GM_IC_pos_net = new_img_like(GM_intersec_pos_neg, np.around(GM_IC_pos_net_data_sub))
plotting.plot_roi(GM_IC_pos_net, cut_coords = [-1, -44, 12], cmap = colors.ListedColormap(['tab:orange', 'tab:brown', 'tab:purple']), colorbar = True)
print(np.unique(get_data(GM_IC_pos_net), return_counts = True))
...
#Energy/WHR ~ GMV + Control
X_WHR1 = np.concatenate((np.array(df_SCE_gmv_2000_SCE[gmv]), np.array(df_SCE_gmv_2000_SCE[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
GMV_train_WHR, GMV_test_WHR, beta_GMV, GMV_models = CV(p_grid = par_grid, out_fold = 5, in_fold = 5, model = ElasticNet(max_iter = 1000000), X = X_WHR1, y = np.array(stats.zscore(df_SCE_gmv_2000_SCE['waist_hip_ratio'])), n_beta = 139, rand = 9)

#Energy/WHR ~ ICA_GT + Control
X_WHR_GT = np.concatenate((stats.zscore(np.array([IC_Graph[i] for i in df_SCE_gmv_2000_SCE.index])), np.array(df_SCE_gmv_2000_SCE[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
WHR_GT_train_E, WHR_GT_test_E, beta_GT, GT_models = CV(p_grid = par_grid, out_fold = 5, in_fold = 5, model = ElasticNet(max_iter = 1000000), X = X_WHR_GT, y = np.array(stats.zscore(df_SCE_gmv_2000_SCE['waist_hip_ratio'])), n_beta = 110, rand = 9)
...
#Energy/WHR ~ GMV + Control
X_WHR1 = np.concatenate((np.array(df_SCE_gmv_2000_WHR[gmv]), np.array(df_SCE_gmv_2000_WHR[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
...
#Energy/WHR ~ ICA_GT + Control
X_WHR_GT = np.concatenate((stats.zscore(np.array([IC_Graph[i] for i in df_SCE_gmv_2000_WHR.index])), np.array(df_SCE_gmv_2000_WHR[['BMI', 'age', 'height', 'hand', 'edu', 'IQ', 'income', 'household'] + list(df_genetic_noNAN.columns[1:])])), axis = 1)
